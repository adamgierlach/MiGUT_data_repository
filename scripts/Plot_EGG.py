...
def read_egg_v2(file,channels=1,header=7,rate=32):
    ...
    voltarray.shape
    timearray.shape
    size=int(voltarray.size/channels)
    print(size)
...
#            print(space)
    ...
    ax_an.vlines(xlim[0]+scale_bar_position_xoffset,ymin=0-3*spacer/4+scale_bar_position_yoffset,ymax=0-spacer/2+scale_bar_position_yoffset,linewidth=scale_linewidth_value,color='black')
...
def egg_freq_heatplot(dat, rate=62.5, xlim=[0,10000],seg_length=500,freq=[0.02,0.2],freqlim=[1,10],vrange=[0,0],figsize=(10,20),interpolation='bilinear',n=10, intermediate=False,max_scale=.4,norm=True):
    '''
...
#        print(channel_freq.shape)
    ...
    for i,ele in enumerate(ax_an):
        ...
        if vrange[1] == 0:
            vrange[1] = freq_data_list[:,:,7-i].max()*max_scale


        colors=ele.imshow(np.flip(freq_data_list[:,:,7-i].T,axis=0),aspect='auto',extent=[xlim[0],xlim[1],freqlim[0],freqlim[1]],cmap='jet',vmin=vrange[0],vmax=vrange[1],interpolation='nearest',resample=True)
