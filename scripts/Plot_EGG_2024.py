...
def egg_signalfreq(dat,rate=62.5,freqlim=[1,10],ylim=0,mode='power',ylog=False,xlog=False,clip=False,labels=[],figsize=(10,20),vline=[],vline_color='black',textsize=12,name_dict={}):
    '''
...
#            print('es')
            if namelist[i-1] in name_dict:
                print('as')
                ax[dat.shape[1]-1-i].set_ylabel(name_dict[namelist[i-1]])
            else:
                ax[dat.shape[1]-1-i].set_ylabel(namelist[i-1])
        elif len(labels) == dat.shape[1]-1:
            if labels[i-1] in name_dict:
                ax[dat.shape[1]-1-i].set_ylabel(name_dict[labels[i-1]])
            else:
        ...
        if len(vline) != 0:
            ymax=ax[dat.shape[1]-1-i].get_ylim()[1]*1.5
            ymin=ax[dat.shape[1]-1-i].get_ylim()[0]
            if ylog: ymax=ymax/1.5*10
            ax[dat.shape[1]-1-i].vlines(vline,ymin,ymax,linewidth=2,color=vline_color,linestyle='dashed')
...
#        ax[i-1].autoscale(axis='y')
        ...
        if ylog: ax[dat.shape[1]-1-i].set_yscale('log')
        if xlog: ax[dat.shape[1]-1-i].set_xscale('log')

    ax[dat.shape[1]-2].set_xlabel('Frequency (1/mins)') # this gives us the bottom graph
    #if mode == 'power': fig.supylabel('Power (au)',pad=10)
    #if mode == 'fft' : fig.supylabel('FFT (au)',pad=10)
    fig.align_ylabels()
